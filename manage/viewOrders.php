							...
							<div class="box-content nopadding">
								<table class="table table-hover table-nomargin dataTable table-bordered">
									...
									<?php foreach ($data as $row){
										$status = getStatus('readByID', $row->getStatus());
										echo '<tr>'."\n";
										echo '	<td>'.$row->getCustomerID().'</td>'."\n";
										echo '	<td>'.$row->getQuantity().'</td>'."\n";
										echo '	<td>'.$status->getName().'</td>'."\n";
										echo '	<td>';

										echo '		<a href="editOrders.php?'.strtolower($singular).'ID='.$row->getID().'" role="button" class="btn">Edit</a>';
